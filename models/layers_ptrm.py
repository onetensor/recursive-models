...
class CrossAttention(nn.Module):
    def __init__(self, hidden_size, head_dim, num_heads, num_key_value_heads, causal=False):
        ...
        if num_heads % num_key_value_heads != 0:
            raise ValueError(f"num_heads ({num_heads}) must be a multiple of num_key_value_heads ({num_key_value_heads}).")
    ...
    def _expand_kv_heads(self, key: torch.Tensor, value: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        key = key.repeat_interleave(self.num_key_value_groups, dim=2)
        value = value.repeat_interleave(self.num_key_value_groups, dim=2)
        return key, value
    ...
    def forward(
        ...
        cos_sin_q: Optional[CosSin] = None,
        cos_sin_k: Optional[CosSin] = None,
    ) -> torch.Tensor:
        batch_size, seq_len_q, _ = hidden_states_q.shape
        _, seq_len_kv, _ = hidden_states_kv.shape

        query = self.q_proj(hidden_states_q).view(batch_size, seq_len_q, self.num_heads, self.head_dim)

        kv = self.kv_proj(hidden_states_kv).view(batch_size, seq_len_kv, 2 * self.num_key_value_heads, self.head_dim)
        key = kv[:, :, :self.num_key_value_heads]
        value = kv[:, :, self.num_key_value_heads:]

        if cos_sin_q is not None:
            cos_q, sin_q = cos_sin_q
            query = apply_rotary_pos_emb_single(query, cos_q, sin_q)
        if cos_sin_k is not None:
            cos_k, sin_k = cos_sin_k
            key = apply_rotary_pos_emb_single(key, cos_k, sin_k)

        key, value = self._expand_kv_heads(key, value)

        query, key, value = map(lambda t: einops.rearrange(t, 'B S H D -> B H S D'), (query, key, value))
        attn_output = scaled_dot_product_attention(query=query, key=key, value=value, is_causal=self.causal)
        attn_output = einops.rearrange(attn_output, 'B H S D -> B S H D')
        attn_output = attn_output.reshape(batch_size, seq_len_q, self.output_size)  # type: ignore
        return self.o_proj(attn_output)
